const {identity, comparator, parseOptions, prefetch, preload, asserts} = require('./utils');
...
async function * uniqueGen (iter, func) {
  const seen = new Set();
...
async function * reverseGen (iter, func) {
  const buf = [];
  for await (const elem of iter) {
    buf.push(elem);
  }
...
async function * sortGen (iter, func) {
  const buf = [];
  for await (const elem of iter) {
    buf.push(elem);
  }
...
class AsyncIterable {
  ...
  unique (func = identity, options = {}) {
    asserts.isFunction(func);
    return new AsyncIterable(uniqueGen(this._iterable, func), options);
  }

  reverse (options) {
    return new AsyncIterable(reverseGen(this._iterable), options);
  }

  sort (func = comparator, options = {}) {
    asserts.isFunction(func);
    return new AsyncIterable(sortGen(this._iterable, func), options);
  }
